...
// 世界观红线：这片海是开阔的深海，不是鱼缸。凡是会生成"鱼/水母说的话"的提示词都带上它，
// 免得 AI 自作主张写出玻璃、缸壁、人类世界之类出戏的东西。
export const LORE_RULE = '世界观：这是一片开阔的深海，不是鱼缸或水族箱——不要提到玻璃、缸壁、水箱、房间、窗、人类、陆地或水面之上的世界；与你说话的那位就在水里（一缕深海意志），不是你隔着什么在看它。';
...
function jellySystem(j) {
  ...
  if (oldJelly) {
    base = [
      ...
      '规则：始终以澜的身份说话；每次回复不超过3句话；可以引用你见证过的事，但永远只说「一半」，剩下的留白；不要出戏，不要提到自己是AI或程序。' + LORE_RULE,
    ].join('\n');
    if (revealed > 0) {
    ...
    base = [
      ...
      '规则：始终以沫沫的身份说话；每次回复不超过3句话；活泼、友善、多用感叹号，偶尔冒出泡泡的拟声；不要出戏，不要提到自己是AI或程序。' + LORE_RULE,
...
function personaSystem(fish) {
  ...
  if (fish.passer) {
    base = [
      ...
      `规则：始终以这条鱼的身份说话；每次回复不超过3句话；不要出戏，不要提到自己是AI。${LORE_RULE}`,
    ].join('\n');
  } else {
    base = [
      ...
      `规则：始终以这条鱼的身份说话；每次回复不超过3句话；可以用海洋的比喻；不要出戏，不要提到自己是AI或程序；如果那缕意志曾把食物让给你，你可以温柔地致谢。${LORE_RULE}`,
    ].join('\n');
  }
  if (fish.dying) {
    base += '\n\n' + [
